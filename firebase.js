...
const firebaseConfig = {
  apiKey: 'AIzaSyBiWBqeEe64_G5TNPPVDTVuR_p-QNqNUY',
...
export async function createAdminAccount(email, password) {
  ...
  if (
    !current ||
    String(current.email || '').toLowerCase() !== ROOT_ADMIN_EMAIL
  ) {
    throw {
      code: 'permission-denied',
      message:
        'Only the Primary Admin (ddu@gmail.com) can create an Admin.'
    };
  }
...
export function firebaseErrorMessage(error) {
  ...
  const map = {
    ...
    'permission-denied':
      'Firestore denied access. Check that the Firestore Rules were published and your user profile exists in users/{UID}.',
